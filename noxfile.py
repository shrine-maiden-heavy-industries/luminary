...
ENABLE_COVERAGE = IN_CI or getenv('LUMINARY_TEST_COVERAGE') is not None
...
def lint(session: Session) -> None:
	...
	session.run(
		'flake8', '--config', str((CNTRB_DIR / '.flake8').resolve()), './luminary_fpga',
	)
...
def upload(session: Session) -> None:
	...
	dist(session)
	session.log(f'Uploading luminary_fpga-{luminary_version()} to PyPi')
	session.run(
		'python', '-m', 'twine',
		'upload', f'{DIST_DIR}/luminary_fpga-{luminary_version()}*'
	)
